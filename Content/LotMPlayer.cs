...
namespace zhashi.Content
{
    public class LotMPlayer : ModPlayer
    {
        ...
        // 1. 序列数据与核心变量
        // ===================================================
        public int currentSequence = 10;       // 巨人/战士途径 (9-1)
        public int currentHunterSequence = 10; // 猎人途径 (9-1)
        public bool IsBeyonder => currentSequence < 10 || currentHunterSequence < 10;
        ...
        // --- 巨人途径技能状态 ---
        public bool dawnArmorActive = false;       // 黎明铠甲 (序列6)
        public bool dawnArmorBroken = false;       // 铠甲破碎状态
        public int dawnArmorCurrentHP = 250;       // 铠甲当前耐久
        public const int DAWN_ARMOR_MAX_HP = 250;  // 铠甲最大耐久
        public int dawnArmorCooldownTimer = 0;     // 破碎冷却
        public const int DAWN_ARMOR_COOLDOWN_MAX = 900; // 15秒冷却

        public bool isGuardianStance = false;      // 守护姿态 (序列5)
        public bool isMercuryForm = false;         // 水银化 (序列3)
        public int twilightResurrectionCooldown = 0; // 黄昏复活冷却 (序列2)
        public const int TWILIGHT_RESURRECTION_MAX = 18000; // 5分钟冷却

        // --- 猎人途径技能状态 ---
        public bool isFlameCloakActive = false;    // 火焰披风 (序列7)
        public bool arsonistFireImmune = false;    // 纵火家火焰免疫
        public bool isFireEnchanted = false;       // 武器附魔 (序列6)
        public int fireTeleportCooldown = 0;       // 闪现冷却 (序列6)
        public const int FIRE_TELEPORT_MAX = 60;   // 1秒冷却

        public int fireballChargeTimer = 0;        // 火球蓄力计时 (序列7)
        public bool isChargingFireball = false;

        public bool isFireForm = false;            // 火焰化 (序列4)
        public bool isArmyOfOne = false;           // 集众 (序列4)
        public bool isCalamityGiant = false;       // 灾祸巨人 (序列2)
        public int glacierCooldown = 0;            // 冰河冷却 (序列2)
        ...
        {
            // =====================================================
            // 1. 巨人/战士途径 (Warrior / Giant Pathway)
            // =====================================================
            ...
            {
                Lighting.AddLight(Player.Center, 1.5f, 1.5f, 1.5f); // 自身发光
                Player.statDefense += 10;
                Player.lifeRegen += 2;
                // 黎明铠甲技能在 ProcessTriggers 处处理
            }
            ...
            if (currentSequence <= 3)
            {
                ...
                Player.blackBelt = true; // 闪避能力
                // 水银化技能逻辑在 PostUpdateMiscEffects
            }

            // 序列 2: 荣耀者 (Glory Sovereign)
            ...
            if (currentSequence <= 1)
            {
                ...
                // 【修复】移除了错误的 Player.scale 代码，避免报错
            }


            // =====================================================
            // 2. 猎人途径 (Hunter / Red Priest Pathway)
            // =====================================================

            // 序列 9: 猎人 (Hunter)
            if (currentHunterSequence <= 9)
            {
            ...
            // 序列 8: 挑衅者 (Provoker)
            if (currentHunterSequence <= 8)
            {
                Player.statDefense += 8;
                Player.aggro += 300; // 增加仇恨
                Player.lifeRegen += 2;
            }
            ...
            if (currentHunterSequence <= 7)
            {
                ...
                Player.buffImmune[BuffID.OnFire3] = true; // 狱火
                Player.buffImmune[BuffID.Frostburn] = true; // 霜火
                Player.resistCold = true;
                // 火球术技能在 ProcessTriggers
            }

            // 序列 6: 阴谋家 (Conspirator)
            if (currentHunterSequence <= 6)
            {
                Player.GetCritChance(DamageClass.Generic) += 10;
                Player.manaCost -= 0.15f; // 减少魔力消耗
                // 闪现技能在 ProcessTriggers
            }
            ...
            if (currentHunterSequence <= 5)
            {
                ...
                // 弱点击破(ModifyHitNPC) 和 斩杀(CheckExecution) 逻辑
            }

            // 序列 4: 铁血骑士 (Iron-blooded Knight)
            if (currentHunterSequence <= 4)
            {
                Player.statDefense += 40;
                Player.endurance += 0.15f;
                Player.maxMinions += 2; // 统御能力
                Player.noKnockback = true;
                // 火焰化技能在 PostUpdateMiscEffects
            }

            // 序列 3: 战争主教 (War Bishop)
            if (currentHunterSequence <= 3)
            {
                Player.maxMinions += 3;
                Player.maxTurrets += 2;
                // 团队增益光环（如果有队友系统可在此添加）
            ...
            if (currentHunterSequence <= 2)
            {
                ...
                Player.buffImmune[BuffID.WindPushed] = true; // 免疫风推
                // 灾祸巨人形态, 雷击, 冰河 技能
            }
            ...
            if (currentHunterSequence <= 1)
            {
                ...
                Player.endurance += 0.30f; // 总计高额免伤
                Player.GetDamage(DamageClass.Generic) += 0.80f; // 80% 独立增伤
                Player.GetCritChance(DamageClass.Generic) += 30;
                Player.aggro += 2000; // 绝对的仇恨吸引
        ...
        private void CheckExecution(NPC target)
        {
            if (currentHunterSequence <= 5)
            {
                // 斩杀逻辑：非Boss单位，血量低于 20%
                if (!target.boss && target.lifeMax > 10 && target.life < target.lifeMax * 0.2f)
                {
                    int executeDamage = target.life + 9999;
                    target.SimpleStrikeNPC(executeDamage, 0, true, 0, DamageClass.Generic, true);
                    CombatText.NewText(target.getRect(), Color.Red, "斩杀!", true);
                    for (int i = 0; i < 30; i++)
                    {
                        int d = Dust.NewDust(target.position, target.width, target.height, DustID.Blood, 0, 0, 0, default, 2.5f);
        ...
        private void CheckRitualKill(NPC target)
        {
            if (target.life <= 0)
            {
                if (currentSequence == 5 && demonHunterRitualProgress < DEMON_HUNTER_RITUAL_TARGET)
                {
                    if (target.type == NPCID.RedDevil)
                    {
                        demonHunterRitualProgress++;
                        if (demonHunterRitualProgress >= DEMON_HUNTER_RITUAL_TARGET)
                        {
                            Main.NewText("【猎魔仪式】已完成！", 200, 0, 255);
                            SoundEngine.PlaySound(SoundID.Roar, Player.position);
                        }
                        else CombatText.NewText(Player.getRect(), Color.Purple, $"猎魔: {demonHunterRitualProgress}/{DEMON_HUNTER_RITUAL_TARGET}");
                    }
                }
                if (currentHunterSequence == 5 && ironBloodRitualProgress < IRON_BLOOD_RITUAL_TARGET)
                {
                    if (Player.slotsMinions >= 5f)
                    {
                        ironBloodRitualProgress++;
                        if (ironBloodRitualProgress >= IRON_BLOOD_RITUAL_TARGET)
                        {
                            Main.NewText("【铁血仪式】已完成！", 255, 69, 0);
                            SoundEngine.PlaySound(SoundID.Roar, Player.position);
                        }
                        else if (ironBloodRitualProgress % 10 == 0)
                        {
                            CombatText.NewText(Player.getRect(), Color.OrangeRed, $"征服: {ironBloodRitualProgress}/{IRON_BLOOD_RITUAL_TARGET}");
                        }
                    }
                }
            }
        ...
        public override void PostHurt(Player.HurtInfo info)
        {
            ...
            {
                dawnArmorCurrentHP -= info.Damage;
                if (dawnArmorCurrentHP <= 0)
                {
                    dawnArmorCurrentHP = 0;
                    dawnArmorActive = false;
                    dawnArmorBroken = true;
                    dawnArmorCooldownTimer = DAWN_ARMOR_COOLDOWN_MAX;
                    SoundEngine.PlaySound(SoundID.Shatter, Player.position);
                    Main.NewText("警告：【黎明铠甲】破碎！", 255, 50, 50);
                }
            }
            ...
            if (currentSequence == 6 && guardianRitualProgress < GUARDIAN_RITUAL_TARGET)
            {
                ...
                {
                    if (Main.npc[i].active && Main.npc[i].townNPC && Main.npc[i].Distance(Player.Center) < 800f)
                    {
                        npcNearby = true;
                        break;
                    }
        ...
        public override bool PreKill(double damage, int hitDirection, bool pvp, ref bool playSound, ref bool genGore, ref PlayerDeathReason damageSource)
        {
            ...
            if (currentSequence <= 2 && twilightResurrectionCooldown <= 0)
            {
                ...
                for (int i = 0; i < 100; i++) { int d = Dust.NewDust(Player.position, Player.width, Player.height, DustID.OrangeTorch, 0, 0, 0, default, 3f); Main.dust[d].noGravity = true; Main.dust[d].velocity *= 3f; }
                ...
                Main.NewText("黄昏的权柄让你拒绝了死亡！(冷却开始)", 255, 100, 0);
                return false;
            }

            // 铁血仪式失败判定
            if (currentHunterSequence == 5 && ironBloodRitualProgress < IRON_BLOOD_RITUAL_TARGET && ironBloodRitualProgress > 0)
            {
                ironBloodRitualProgress = 0;
                Main.NewText("失败... 统帅不应倒下。仪式进度已重置。", 255, 50, 50);
            }
            return true;
        }
        ...
        public override void PostUpdateMiscEffects()
        {
            ...
            if (isCalamityGiant)
            {
                if (!TryConsumeSpirituality(100.0f, true))
                {
                    isCalamityGiant = false;
                    Main.NewText("灵性枯竭，灾祸形态解除！", 255, 50, 50);
                    return;
                }
                Player.statDefense += 200; Player.endurance += 0.3f; Player.GetDamage(DamageClass.Generic) += 1.0f;
                Player.moveSpeed += 2f; Player.maxRunSpeed += 10f; Player.noKnockback = true;
                Player.wingsLogic = 0; Player.wingTime = 9999; Player.gravity = 0f;
                Player.statLifeMax2 += 2000; Player.invis = true;
                if (Main.rand.NextBool(2)) Dust.NewDust(Player.position, Player.width, Player.height, DustID.Electric, 0, 0, 0, default, 1.5f);
            }

            // 集众 (铁血骑士)
            if (isArmyOfOne)
            {
                if (currentHunterSequence <= 4 && TryConsumeSpirituality(5.0f, true))
                {
                    Player.maxMinions += 5;
                }
                else isArmyOfOne = false;
            }
            ...
            if (isFireForm)
            {
                ...
                Player.noKnockback = true;
                Player.wingsLogic = 0;
                Player.wingTime = 9999;
                Player.rocketTime = 9999;
                Player.noItems = true;

                if (currentHunterSequence <= 1) // 序列1：征服者紫焰长枪
                {
                    Player.moveSpeed += 5.0f;
                    Player.maxRunSpeed += 30f;
                    Player.runAcceleration *= 10f;
                    Player.jumpSpeedBoost += 30f;
                    Player.statDefense += 200;
                    Player.endurance += 0.4f;

                    // 紫色火焰视觉效果
                    Vector2 tip = Player.Center + Player.velocity * 2f;
                    for (int i = 0; i < 5; i++)
                    {
                        Vector2 pos = Vector2.Lerp(Player.Center, tip, i / 5f) + Main.rand.NextVector2Circular(10, 10);
                        int d = Dust.NewDust(pos, 0, 0, DustID.Shadowflame, 0, 0, 0, default, 2.5f);
                        Main.dust[d].noGravity = true;
                        Main.dust[d].velocity = -Player.velocity * 0.2f;
                    }

                    // 【修复】扩大判定范围
                    Rectangle myRect = Player.getRect();
                    myRect.Inflate(40, 40);
                    ...
                    if (Player.velocity.Length() > 5f)
                    {
                        ...
                        {
                            NPC npc = Main.npc[i];
                            if (npc.active && !npc.friendly && !npc.dontTakeDamage && npc.getRect().Intersects(myRect))
                            {
                                if (npc.immune[Player.whoAmI] <= 0)
                                {
                                    int dashDamage = (int)(Player.GetDamage(DamageClass.Melee).ApplyTo(2000));

                                    Player.ApplyDamageToNPC(npc, dashDamage, 30f, Player.direction, true);
                                    npc.AddBuff(ModContent.BuffType<ConquerorWill>(), 600);
                                    npc.immune[Player.whoAmI] = 6;

                                    SoundEngine.PlaySound(SoundID.Item74, npc.Center);
                                    for (int k = 0; k < 20; k++) Dust.NewDust(npc.position, npc.width, npc.height, DustID.Shadowflame, 0, 0, 0, default, 3f);
                                }
                            }
                        }
                    }
                ...
                else // 序列4：普通火焰
                {
                    ...
                    Player.gravity *= 0.5f;
                    Player.statDefense += 100;
                    for (int i = 0; i < 3; i++) { int d = Dust.NewDust(Player.position, Player.width, Player.height, DustID.SolarFlare, 0, 0, 0, default, 2f); Main.dust[d].noGravity = true; Main.dust[d].velocity = Player.velocity * 0.5f; }
                }
            }
            ...
            if (isMercuryForm)
            {
                if (!TryConsumeSpirituality(20.0f, true)) { isMercuryForm = false; Main.NewText("灵性枯竭，被迫退出水银形态！", 255, 50, 50); return; }
                Player.moveSpeed += 2.0f; Player.maxRunSpeed += 10f; Player.runAcceleration *= 3f; Player.noKnockback = true; Player.invis = true;
                Rectangle myRect = Player.getRect(); myRect.Inflate(10, 10); foreach (NPC npc in Main.npc) { if (npc.active && !npc.friendly && !npc.dontTakeDamage && npc.getRect().Intersects(myRect)) { if (npc.immune[Player.whoAmI] == 0) { int damage = (int)(Player.GetDamage(DamageClass.Melee).ApplyTo(50) * 5f); Player.ApplyDamageToNPC(npc, damage, 10f, Player.direction, false); npc.immune[Player.whoAmI] = 10; npc.AddBuff(BuffID.Slow, 300); npc.AddBuff(BuffID.Frostburn, 300); } } }
            }

            // 潜行
            if (currentSequence <= 3 && !isMercuryForm)
            {
                if (Player.velocity.Length() < 0.1f) { stealthTimer++; if (stealthTimer > 60) { Player.invis = true; Player.aggro -= 1000; } } else { stealthTimer = 0; }
            }

            // 守护姿态
            if (isGuardianStance)
            {
                Player.velocity.X = 0; if (Player.velocity.Y < 0) Player.velocity.Y = 0; Player.noItems = true; Player.statDefense += 80; Player.endurance += 0.5f; Player.noKnockback = true; Player.aggro += 1000; Player.hasPaladinShield = true;
                if (Player.ownedProjectileCounts[ModContent.ProjectileType<GuardianShieldProjectile>()] <= 0) Projectile.NewProjectile(Player.GetSource_FromThis(), Player.Center, Vector2.Zero, ModContent.ProjectileType<GuardianShieldProjectile>(), 0, 0, Player.whoAmI);
            }
        }
            ...
            if (LotMKeybinds.FireForm.JustPressed)
            {
                ...
                else if (currentHunterSequence <= 4)
                {
                    ...
                    if (isFireForm)
                    {
                        ...
                        if (currentHunterSequence <= 1) Main.NewText("征服者：紫焰长枪 (毁灭一切阻挡者)", 180, 0, 255);
                        else Main.NewText("铁血骑士：元素化 (禁用武器)", 255, 69, 0);
                    }
                    else Main.NewText("解除形态", 200, 200, 200);
                }
            }

            // U键：雷击 (序列2)
            if (LotMKeybinds.WeatherStrike.JustPressed && currentHunterSequence <= 2)
            {
                if (TryConsumeSpirituality(200, true))
                {
                    Projectile.NewProjectile(Player.GetSource_FromThis(), Main.MouseWorld, Vector2.Zero, ModContent.ProjectileType<WeatherStrikeLightning>(), 500, 5f, Player.whoAmI);
                }
            }

            // I键：冰河 (序列2)
            if (LotMKeybinds.GlacierFreeze.JustPressed && currentHunterSequence <= 2)
            {
                if (glacierCooldown <= 0 && TryConsumeSpirituality(1000, true))
                {
                    Main.NewText("冰河世纪！", 0, 255, 255);
                    SoundEngine.PlaySound(SoundID.Item28, Player.position);
                    GlacierVisualSystem.StartFlash();
                ...
                else if (glacierCooldown > 0) Main.NewText($"冰河冷却中: {glacierCooldown / 60}s", 200, 50, 50);
            }
        ...
        private void CheckConquerorRitual()
        {
            if (currentHunterSequence == 2 && !conquerorRitualComplete && ConquerorSpawnSystem.StopSpawning)
            {
                ...
                {
                    NPC npc = Main.npc[i];
                    if (npc.active && !npc.friendly && !npc.townNPC && npc.lifeMax > 5 && !npc.dontTakeDamage) { enemyExists = true; break; }
                }
        ...
        // 灵性消耗检测
        public bool TryConsumeSpirituality(float amount, bool isMaintenance = false)
        {
            if (isCalamityGiant && !isMaintenance) return true;
            if (spiritualityCurrent >= amount) { spiritualityCurrent -= amount; return true; }
            return false;
        }
        ...
        {
            if (isCalamityGiant || isFireForm || isMercuryForm) { drawInfo.hideEntirePlayer = true; return; }
            if (currentSequence <= 3 && stealthTimer > 60) { drawInfo.hideEntirePlayer = true; drawInfo.shadow = 0f; a = 0f; return; }
            if (currentSequence <= 6 && dawnArmorActive && !dawnArmorBroken && !Player.shroomiteStealth)
            {
                r = 2.0f; g = 2.0f; b = 2.0f; fullBright = true;
                if (Main.rand.NextBool(20)) Dust.NewDustPerfect(Player.Center, DustID.Enchanted_Gold, Vector2.Zero, 0, default, 0.8f).noGravity = true;
            }
        }

        public override bool CanUseItem(Item item)
        {
            if (isFireForm || isMercuryForm || isGuardianStance) return false;
        ...
        {
            if (isCalamityGiant && Main.rand.NextFloat() < 0.5f) { Player.SetImmuneTimeForAllTypes(60); return true; }
            if (isMercuryForm && Main.rand.NextFloat() < 0.75f) { Player.SetImmuneTimeForAllTypes(60); return true; }
        ...
        {
            int baseSpirit = 100;
            if (currentSequence <= 9) baseSpirit = 200;
            if (currentSequence <= 8) baseSpirit = 350;
            if (currentSequence <= 7) baseSpirit = 600;
            if (currentSequence <= 6) baseSpirit = 1200;
            if (currentSequence <= 5) baseSpirit = 2500;
            if (currentSequence <= 4) baseSpirit = 5000;
            if (currentSequence <= 3) baseSpirit = 10000;
            if (currentSequence <= 2) baseSpirit = 50000;

            int hunterSpirit = 100;
            if (currentHunterSequence <= 9) hunterSpirit = 200;
            if (currentHunterSequence <= 8) hunterSpirit = 350;
            if (currentHunterSequence <= 7) hunterSpirit = 600;
            if (currentHunterSequence <= 6) hunterSpirit = 1200;
            if (currentHunterSequence <= 5) hunterSpirit = 2500;
            if (currentHunterSequence <= 4) hunterSpirit = 6000;
            if (currentHunterSequence <= 3) hunterSpirit = 15000;
            if (currentHunterSequence <= 2) hunterSpirit = 50000;
            if (currentHunterSequence <= 1) hunterSpirit = 100000;

            if (hunterSpirit > baseSpirit) baseSpirit = hunterSpirit;
            spiritualityMax = baseSpirit;
        ...
        private void HandleSpiritualityRegen()
        {
            ...
            if (spiritualityRegenTimer >= 60)
            {
                ...
                if (currentSequence <= 4) regenAmount += 30;
                if (currentSequence <= 3) regenAmount += 50;
                if (currentSequence <= 2) regenAmount += 200;

                if (currentHunterSequence <= 9) regenAmount += 1f;
                if (currentHunterSequence <= 6) regenAmount += 3f;
                if (currentHunterSequence <= 5) regenAmount += 5f;
                if (currentHunterSequence <= 4) regenAmount += 20f;
                if (currentHunterSequence <= 3) regenAmount += 50f;
                if (currentHunterSequence <= 2) regenAmount += 200f;
                if (currentHunterSequence <= 1) regenAmount += 500f;
